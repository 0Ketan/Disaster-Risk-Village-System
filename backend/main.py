...
Executes startup health diagnostics across OpenTopoData and Meteostat,
...
def run_startup_diagnostics():
    ...
    print(f" [1/2] OpenTopoData (Elevation):   {mode_str:<10} ({topo_health.message}, {lat_str})")
    ...
    print(f" [2/2] Meteostat (Climate Data):   {mode_str:<10} ({meteo_health.message}, {lat_str})")

    print("----------------------------------------------------------------------")
    print(" Status: OPERATIONAL (Resilient Fallbacks Active)")
    print(" Server running on http://127.0.0.1:5000 (Swagger docs at /docs)")
    print("======================================================================")


def main():
    run_startup_diagnostics()
    # Changed port to 5000 to fix Windows Errno 10048 phantom port bug and match Vite proxy
